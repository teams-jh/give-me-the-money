 ...
 */
export function detectGoldenCross(closes: number[], lookback: number = 5): CrossSignal {
  const alerts: Alert[] = [];
    ...
    if (rsiWindow.length >= 4) {
      ...
      if (priceEnd >= priceHigh * 0.99 && rsiEnd < rsiStart - 5) {
        alerts.push({
          ...
          direction: 'bearish',
          strength: 'normal',
          label: `RSI 하락 다이버전스 (가격↑ RSI↓)`,
          value: latestRSI ?? undefined,
          scoreAffecting: true,
        });
      }
 ...
 */
export function detectMACDCross(closes: number[], lookback: number = 5): MACDCrossSignal {
...
export function detectRiskSignal(
  ...
  const stopLoss =
    latestATR !== null && latestPrice !== null
...
export function detectStochasticSignal(
  ...
  if (latestK !== null) {
    if (latestK > 90) {
      alerts.push({
        type: 'stoch_extreme_overbought',
        direction: 'bearish',
        strength: 'strong',
        label: `Stochastic 극단 과매수 (%K ${latestK})`,
        value: latestK,
        scoreAffecting: true,
      });
    } else if (latestK > 80) {
      alerts.push({
        ...
        direction: 'bearish',
        strength: 'normal',
        label: `Stochastic 과매수 (%K ${latestK})`,
        value: latestK,
        scoreAffecting: true,
      });
    } else if (latestK < 10) {
      alerts.push({
        type: 'stoch_extreme_oversold',
        direction: 'bullish',
        strength: 'strong',
        label: `Stochastic 극단 과매도 (%K ${latestK})`,
        value: latestK,
        scoreAffecting: true,
      });
    } else if (latestK < 20) {
      alerts.push({
        ...
        direction: 'bullish',
        strength: 'normal',
        label: `Stochastic 과매도 (%K ${latestK})`,
        value: latestK,
        scoreAffecting: true,
      });
    }
  ...
  if (n >= divLook + 14) {
    ...
    if (kWin.length >= 4) {
      ...
      if (priceEnd >= priceMax * 0.99 && kEnd < kStart - 5) {
        alerts.push({
          type: 'stoch_bearish_divergence',
          direction: 'bearish',
          strength: 'normal',
          label: `Stochastic 하락 다이버전스 (가격↑ %K↓)`,
          value: latestK ?? undefined,
          scoreAffecting: true,
        });
      }
      if (priceEnd <= priceMin * 1.01 && kEnd > kStart + 5) {
        alerts.push({
          type: 'stoch_bullish_divergence',
          direction: 'bullish',
          strength: 'normal',
          label: `Stochastic 상승 다이버전스 (가격↓ %K↑)`,
          value: latestK ?? undefined,
          scoreAffecting: true,
        });
      }
...
export function detectROCSignal(closes: number[]): ROCSignal {
  ...
  const directions = [roc5, roc20, roc60]
    .filter((v): v is number => v !== null)
    .map((v) => (v > 0 ? 1 : v < 0 ? -1 : 0));
  ...
  if (bullCount === total) {
    ...
    const desc = [
      roc5 !== null ? `5d:${roc5 > 0 ? '+' : ''}${roc5.toFixed(1)}%` : '',
      roc20 !== null ? `20d:${roc20 > 0 ? '+' : ''}${roc20.toFixed(1)}%` : '',
      roc60 !== null ? `60d:${roc60 > 0 ? '+' : ''}${roc60.toFixed(1)}%` : '',
    ]
      .filter(Boolean)
      .join(' ');
    alerts.push({
      ...
      direction: 'bullish',
      strength: 'normal',
      label: `ROC 상승 모멘텀 (${desc})`,
      scoreAffecting: true,
    });
  } else if (bearCount === total) {
    alerts.push({
      type: 'roc_strong_bearish',
      direction: 'bearish',
      strength: 'strong',
 ...
 */
export function detectMFISignal(ohlcv: OHLCV[], rsiArr?: (number | null)[]): MFISignal {
  const alerts: Alert[] = [];
  ...
  if (latestMFI !== null) {
    if (latestMFI > 90) {
      alerts.push({
        type: 'mfi_extreme_overbought',
        direction: 'bearish',
        strength: 'strong',
        label: `MFI 극단 과매수 (${latestMFI})`,
        value: latestMFI,
        scoreAffecting: true,
      });
      ...
      alerts.push({
        ...
        direction: 'bearish',
        strength: 'normal',
        label: `MFI 과매수 (${latestMFI})`,
        value: latestMFI,
        scoreAffecting: true,
      });
    } else if (latestMFI < 10) {
      alerts.push({
        type: 'mfi_extreme_oversold',
        direction: 'bullish',
        strength: 'strong',
        label: `MFI 극단 과매도 (${latestMFI})`,
        value: latestMFI,
        scoreAffecting: true,
      });
 ...
 */
export function detectSupertrendSignal(ohlcv: OHLCV[], lookback: number = 3): SupertrendSignal {
  const alerts: Alert[] = [];
