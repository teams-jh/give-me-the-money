        ...
        <Stack spacing={3}>
              ...
              <Stack spacing={2}>
                <Box
                  sx={{
                    p: 2,
                    borderRadius: 1,
                    bgcolor: alpha(trendColor, 0.05),
                    border: `1px solid ${alpha(trendColor, 0.2)}`,
                  }}
                >
                  <Typography variant="overline" sx={{ color: trendColor, fontWeight: 800 }}>
                    현재 추세
                  </Typography>
                ...
                <Box
                  sx={{ p: 2, borderRadius: 1, bgcolor: alpha(theme.palette.primary.main, 0.05) }}
                >
                  ...
                  <Typography
                    variant="h5"
                    sx={{ color: isPositive ? 'success.main' : 'error.main', fontWeight: 800 }}
                  >
                    {isPositive ? '+' : ''}
                    {data.total_return}%
                  </Typography>
                ...
                <Box sx={{ p: 2, borderRadius: 1, bgcolor: alpha(theme.palette.grey[500], 0.05) }}>
                  ...
                  <Stack spacing={1} sx={{ mt: 1 }}>
                    <Stack direction="row" justifyContent="space-between">
                      ...
                      <Typography
                        variant="subtitle2"
                        sx={{ color: data.slope_pct > 0 ? 'success.main' : 'error.main' }}
                      >
                        {data.slope_pct > 0 ? '+' : ''}
                        {data.slope_pct.toFixed(3)}
                      </Typography>
                    </Stack>
                    <Stack direction="row" justifyContent="space-between">
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.6 }}>
              분석 기간 동안의 전반부 기울기는{' '}
              <b>
                {data.slope_early_pct > 0 ? '▲ +' : '▼ '}
                {data.slope_early_pct}%
              </b>
              이며, 후반부 기울기는{' '}
              <b>
                {data.slope_late_pct > 0 ? '▲ +' : '▼ '}
                {data.slope_late_pct}%
              </b>
              입니다.
              {data.trend === 'recovering' &&
                ' 전반 하락 후 전반에 반등하는 모습을 보이고 있습니다.'}
