...
export function StockMarketIndexView() {
  ...
  const selectedIndex = useMemo(
    () => INDEX_OPTIONS.find((opt) => opt.id === selectedIndexId) || INDEX_OPTIONS[0],
    [selectedIndexId]
  );
  ...
  const stocksWithReturn = useMemo(() => {
    ...
    return stockData.stocks
      .map((stock) => {
        ...
        const startPrice =
          actualPrices[0] !== undefined ? actualPrices[0] : dataPoints[0] || currentPrice;
        const prevPrice =
          idx > 0
            ? actualPrices[idx - 1] !== undefined
              ? actualPrices[idx - 1]
              : dataPoints[idx - 1]
            : startPrice;
  ...
  const thresholds = useMemo(() => {
    ...
    switch (currentPeriod) {
      case '3m':
        return { strong: 15, medium: 8, light: 3 };
      case '1y':
        return { strong: 30, medium: 15, light: 5 };
      case '2y':
        return { strong: 50, medium: 25, light: 8 };
      case '3y':
        return { strong: 80, medium: 40, light: 12 };
      default:
        return { strong: 30, medium: 15, light: 5 };
    }
  ...
  const getHeatmapColor = useCallback(
    (value: number) => {
      ...
      return {
        bg: theme.palette.action.disabledBackground || '#eaeaea',
  ...
  const filteredHeatmapStocks = useMemo(() => {
    ...
    return stocksWithReturn.filter(
      (item) => (item.stock.sector || 'Others') === selectedHeatmapSector
    );
  ...
  const renderTooltip = useCallback(
    ({ node }: any) => {
      ...
      const isKospiKosdaq =
        selectedIndexId.startsWith('kospi') || selectedIndexId.startsWith('kosdaq');
        ...
        >
          ...
          <Stack spacing={0.25}>
            <Typography
              ...
              sx={{
                fontWeight: 700,
                color: data.cumulativeReturn >= 0 ? 'success.main' : 'error.main',
              }}
            >
              누적 수익률: {data.cumulativeReturn >= 0 ? '+' : ''}
              {data.cumulativeReturn.toFixed(2)}%
            </Typography>
            <Typography
              variant="caption"
              sx={{ fontWeight: 700, color: data.dailyReturn >= 0 ? 'success.main' : 'error.main' }}
            >
              당일 변동률: {data.dailyReturn >= 0 ? '+' : ''}
              {data.dailyReturn.toFixed(2)}%
            </Typography>
          </Stack>
        </Card>
      );
        ...
        >
          <Box>
            ...
            <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
              다양한 글로벌 지수의 구성 종목에 대한 다차원 회귀분석 추세 지표 및 백테스트 히트맵
              타임라인 시뮬레이션을 제공합니다.
            </Typography>
          </Box>
          ...
          >
            <Box sx={{ flexGrow: 1 }}>
              <Typography
                variant="caption"
                sx={{ display: 'block', color: 'text.secondary', fontWeight: 700, mb: 0.5 }}
              >
                분석 대상 주가지수 선택
              </Typography>
              <TextField
              ...
              <Box sx={{ mt: 0.5, display: 'flex', justifyContent: 'space-between' }}>
                <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                  {selectedIndex.description} • 총 {stockData.stocks.length}개 종목 분석 중 • 출처:{' '}
                  {selectedIndex.data.source}
                </Typography>
              </Box>
            </Box>
          </Card>
