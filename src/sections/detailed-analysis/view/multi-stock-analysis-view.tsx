...
const getTextColorForRatio = (ratio: number) => ratio < 0.25 || ratio > 0.75 ? '#ffffff' : '#111111';
...
const METRICS = [
  ...
  // 3. 거래량 및 유동성
  {
    ...
    description: '최근 3개월간 하루 평균 거래량입니다. 평소 이 주식이 얼마나 활발히 거래되는지 보여줍니다.',
    unit: 'number',
    category: '거래량 및 유동성',
  },
  {
    ...
    description: '최근 10일간 하루 평균 거래량입니다. 최근 들어 거래량이 급변했는지 파악할 수 있습니다.',
    unit: 'number',
    category: '거래량 및 유동성',
  },
  // 4. 가치 평가
  ...
  },
  {
    ...
    description: 'PEG 비율 (주가수익성장성비율). PER을 이익성장률로 나눈 값으로, 낮을수록 성장성에 비해 주가가 저렴하다는 뜻입니다.',
    unit: 'number',
    category: '가치 평가',
  },
  {
    ...
    description: 'PBR (주가순자산비율). 회사가 망해서 당장 재산을 청산할 때의 장부 가치 대비 주가입니다.',
    unit: 'number',
    category: '가치 평가',
  },
  {
    ...
    description: 'EPS (주당순이익). 회사가 벌어들인 과거 1년 총 순이익을 총 주식 수로 나눈 값입니다.',
    unit: 'number',
    category: '가치 평가',
  },
  {
  ...
  },
  {
    ...
    label: 'EV (기업가치)',
    description: 'EV (기업가치). 시가총액에 순차입금(갚아야 할 빚 - 쥐고 있는 현금)을 합친 진짜 인수 가격입니다.',
    unit: 'currency',
    category: '가치 평가',
  },
  // 5. 수익성 및 성장성
  ...
  },
  {
    value: 'roe',
    label: 'ROE (자기자본이익률)',
    description: '자기자본이익률. 주주들이 투자한 돈(자본)을 이용해 1년간 몇 %의 이익을 굴렸는지 보여주는 투자 효율성 지표입니다.',
    unit: 'percent',
    category: '수익성 및 성장성',
  },
  {
  ...
  },
  {
    ...
    description: '배당수익률. 현재 주가 기준으로 주식을 샀을 때, 은행 이자처럼 1년에 받을 수 있는 예상 수익률입니다.',
    unit: 'percent',
    category: '배당 정보',
  },
  {
...
export function MultiStockAnalysisView() {
  ...
  const treeData = useMemo(() => {
    ...
    const higherIsBetter = metric?.unit === 'percent' || metric?.value === 'market_cap' || metric?.value === 'roe' || metric?.value === 'roa' || metric?.value === 'revenue_growth' || metric?.value === 'earnings_growth' || metric?.value === 'dividend_yield' || metric?.value === 'held_pct_institutions' || metric?.value === 'held_pct_insiders';
    ...
    const normalizedChildren = children.map((c) => {
      ...
      const textColor = c.val === null || c.val === undefined ? '#888888' : getTextColorForRatio(ratio);
  ...
  const selectedOptions = useMemo(() => selectedTickers.map(
      (t) => tickerOptions.find((o) => o.ticker === t) || { ticker: t, name: t }
    ), [selectedTickers, tickerOptions]);

  const chartSeries = useMemo(() => selectedTickers
      .map((ticker) => {
        const data = allTickersData[ticker];
      ...
      .filter((s) => s !== null) as any[], [selectedTickers, period, startDate, endDate]);
  ...
  const rankingData = useMemo(() => selectedTickers
      .map((ticker) => {
        const val = getMetricValue(ticker, selectedMetric);
        const data = allTickersData[ticker];
        return {
        ...
        <Card sx={{ p: 3, boxShadow: theme.customShadows?.card }}>
          <Stack spacing={3}>
            ...
            {viewMode === 'list' ? (
              <>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                  {Object.entries(groupedMetrics).map(([category, items]) => (
                    <Box key={category}>
                      <Typography variant="subtitle2" sx={{ color: 'primary.main', mb: 1.2, fontWeight: 700 }}>
                        🏷️ {category}
                      </Typography>
              ...
              <Box sx={{ height: 500, width: '100%', bgcolor: 'background.neutral', borderRadius: 1.5, overflow: 'hidden', p: 1, mt: 2 }}>
                {isMounted ? (
                  selectedTickers.length > 0 ? (
                    <ResponsiveTreeMap
                      ...
                      labelSkipSize={16}
                      label={(node: any) => node.data.name ? `${node.data.name} (${node.data.formattedValue})` : node.id}
