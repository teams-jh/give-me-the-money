...
import type { BarUnit, SimResult, PeriodConfig, PriceDataPoint } from 'src/library/shared/trendSim';
...
export type { BarUnit, PeriodConfig };
...
const DEFAULT_CONFIG: Omit<
  PeriodConfig,
  'trendStartDate' | 'trendEndDate' | 'filterStartDate' | 'filterEndDate'
> = {
  barUnit: 'daily',
  trendBase: 'highlow',
  trendAlgo: 'swing',
    ...
    setTimeout(() => {
      ...
      for (const p of simPeriods) {
        ...
        for (const opt of tickerOptions) {
          ...
          const resolvedCfg = resolvePeriodDates(
            cfg,
            slice.map((d) => d.date)
          );
