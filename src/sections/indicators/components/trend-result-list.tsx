...
const PERIOD_LABEL: Record<string, string> = {
  '3m': '3개월', '1y': '1년', '2y': '2년', '3y': '3년',
};

// ----------------------------------------------------------------------

function getMiniChartOptions(
  result: TrendSimFinalResult<SimResult>,
  theme: any
) {
  ...
  pointsToUse.forEach(tp => {
    ...
    const color = isBreak
      ? (isClose ? theme.palette.secondary.main   : theme.palette.secondary.light)
      : (isClose ? theme.palette.error.main       : theme.palette.warning.main);
  ...
  return {
    ...
    stroke:   { curve: ['straight', 'straight', 'straight'] as any, width: [1, 2, 2], dashArray: [0, 4, 4] },
    plotOptions: {
      candlestick: {
...
export function TrendResultList({ sim }: Props) {
  ...
  if (isSimulating) {
    return (
      <Card sx={{ p: 4, textAlign: 'center', border: `1px solid ${theme.palette.divider}`, borderRadius: 2 }}>
        <Typography variant="h6" sx={{ color: 'text.secondary' }}>⏳ 시뮬레이션 실행 중...</Typography>
      </Card>
    );
  }
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="subtitle2" sx={{ color: 'text.secondary' }}>
        총 <strong style={{ color: theme.palette.primary.main }}>{finalResults.length}개</strong> 종목
        {simPeriods.length > 1 && (
          ...
          return (
            <Grid key={result.ticker} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
              <Card sx={{
                ...
                '&:hover': { boxShadow: theme.customShadows?.z8, transform: 'translateY(-2px)', transition: 'all 0.2s' },
              }}>
                <Stack spacing={1.5}>
                  {/* 헤더: 티커 + 기울기 배지 */}
                  <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                    ...
                    <Chip
                      label={
                        result.longestPeriodResult.slopeType === 'positive' ? '↗ 양의 기울기'
                        : result.longestPeriodResult.slopeType === 'negative' ? '↘ 음의 기울기'
                        : '→ 횡보'
                      }
                      size="small"
                      color={
                        result.longestPeriodResult.slopeType === 'positive' ? 'error'
                        : result.longestPeriodResult.slopeType === 'negative' ? 'info'
                        : 'default'
                      }
                    ...
                    {simPeriods.map(p => {
                      ...
                      return (
                        <Stack key={p} direction="row" spacing={1} alignItems="center">
                          ...
                          <Chip label={`터치 ${stat.touchCount}`}    size="small" color="warning" variant="outlined" sx={{ fontSize: '0.65rem', height: 20 }} />
                          <Chip label={`돌파 ${stat.breakoutCount}`} size="small" color="error"   variant="outlined" sx={{ fontSize: '0.65rem', height: 20 }} />
                        </Stack>
                      );
                    })}
                  </Stack>
